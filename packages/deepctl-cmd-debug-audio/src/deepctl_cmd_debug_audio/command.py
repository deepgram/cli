...
class AudioCommand(BaseCommand):
    ...
    def get_arguments(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "names": ["--ffprobe-args"],
                "help": "Custom ffprobe arguments (e.g., '-show_streams -show_format')",
                "type": str,
                "is_option": True
            }
        ]
    ...
    def run_ffprobe(self, file_path: str, custom_args: Optional[str] = None) -> Dict[str, Any]:
        ...
        if "format" in probe_data:
            ...
            audio_info.format = AudioFormat(
                ...
                duration=float(format_data.get("duration", 0)
                               ) if format_data.get("duration") else None,
                size=int(format_data.get("size", 0)
                         ) if format_data.get("size") else None,
                bit_rate=format_data.get("bit_rate"),
                nb_streams=int(format_data.get("nb_streams", 0)
                               ) if format_data.get("nb_streams") else None
            )
            ...
            for stream in probe_data["streams"]:
                if stream.get("codec_type") == "audio":
                    audio_stream = AudioStream(
                        ...
                        duration=float(stream.get("duration", 0)) if stream.get(
                            "duration") else None,
    ...
    def display_basic_info(self, audio_info: AudioInfo):
        ...
        if audio_info.format:
            ...
            console.print(
                f"  • Format: {audio_info.format.format_long_name or audio_info.format.format_name or 'Unknown'}")
        ...
        if audio_info.streams:
            ...
            for i, stream in enumerate(audio_info.streams):
                if len(audio_info.streams) > 1:
                    console.print(f"\n  Stream {i + 1}:")
                console.print(
                    f"  • Codec: {stream.codec_long_name or stream.codec_name or 'Unknown'}")
                if stream.sample_rate:
                    console.print(f"  • Sample Rate: {stream.sample_rate} Hz")
                if stream.channels:
                    console.print(
                        f"  • Channels: {stream.channels} ({stream.channel_layout or 'Unknown layout'})")
                if stream.bit_rate:
    ...
    def display_verbose_info(self, audio_info: AudioInfo):
        ...
        if audio_info.streams:
            for i, stream in enumerate(audio_info.streams):
                stream_table = Table(
                    title=f"Audio Stream {i + 1}" if len(
                        audio_info.streams) > 1 else "Audio Stream",
                    box=box.ROUNDED
                )
            ...
            if audio_info.streams:
                for stream in audio_info.streams:
                    ...
                    if stream.channels and stream.channels > 2:
                        compatibility_issues.append(
                            f"⚠️  Multi-channel audio ({stream.channels} channels) - Consider converting to mono or stereo"
                        )
