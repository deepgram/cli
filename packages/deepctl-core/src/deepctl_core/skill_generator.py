...
def render_developer_guide(
    ...
    *,
    ...
    lines.append("")
    lines.append("Voices follow the naming pattern `aura-2-{name}-en`. Examples:")
    ...
    lines.append("")
    lines.append("### TTS Example (Python)")
    lines.append("")
    lines.append("```python")
    lines.append("from deepgram import DeepgramClient, SpeakOptions")
    lines.append("")
    lines.append('dg = DeepgramClient("DEEPGRAM_API_KEY")')
    lines.append("")
    lines.append("options = SpeakOptions(")
    lines.append('    model="aura-2-andromeda-en"')
    lines.append(")")
    lines.append("")
    lines.append('response = dg.speak.rest.v("1").save(')
    lines.append('    "output.mp3",')
    lines.append('    {"text": "Hello from Deepgram!"},')
    lines.append("    options,")
    lines.append(")")
    lines.append("```")
    lines.append("")
    lines.append("### TTS Example (JavaScript)")
    lines.append("")
    lines.append("```javascript")
    lines.append('import { createClient } from "@deepgram/sdk";')
    lines.append("")
    lines.append('const dg = createClient("DEEPGRAM_API_KEY");')
    lines.append("")
    lines.append("const response = await dg.speak.request(")
    lines.append('  { text: "Hello from Deepgram!" },')
    lines.append('  { model: "aura-2-andromeda-en" }')
    lines.append(");")
    lines.append("")
    lines.append("const stream = await response.getStream();")
    lines.append("// Write stream to file or audio output")
    lines.append("```")
    lines.append("")
    ...
    lines.append("dg transcribe audio.wav     # Transcribe a file")
    lines.append("dg transcribe --stream mic  # Live transcription from mic")
