...
Environment Variables:
  ...
  GITHUB_REPOSITORY - owner/repo (auto-set by Actions; falls back to deepgram/cli)
  DRY_RUN - Set to "false" to actually trigger workflows (default: true for safety)
  MAX_ISSUE_NUMBER - Only process issues with numbers less than this value (default: 99999)
  MIN_ISSUE_NUMBER - Only process issues with numbers >= this value (default: 1)`);
  }
  console.log("[DEBUG] GitHub token found");

  const repoSlug = process.env.GITHUB_REPOSITORY || "deepgram/cli";
  const [owner, repo] = repoSlug.split("/");
  if (!owner || !repo) {
    throw new Error(
      `GITHUB_REPOSITORY must be owner/repo format, got "${repoSlug}"`,
    );
  }
  const dryRun = process.env.DRY_RUN !== "false";
  const maxIssueNumber = parseInt(process.env.MAX_ISSUE_NUMBER || "99999", 10);
  ...
  while (true) {
    ...
    if (oldestIssueInPage && oldestIssueInPage.number >= maxIssueNumber) {
      ...
      oldestIssueInPage &&
      oldestIssueInPage.number < minIssueNumber
    ) {
      console.log(
