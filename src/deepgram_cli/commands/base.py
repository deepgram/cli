...
class BaseCommand(ABC):
    ...
    def execute(self, ctx: click.Context, **kwargs) -> None:
        ...
        if self.requires_project:
            ...
            if not project_id:
                console.print("[red]Error:[/red] Project ID is required for this command")
                console.print("Set DEEPGRAM_PROJECT_ID environment variable or configure via profile")
    ...
    def _output_table(self, result: Any) -> None:
        ...
        if isinstance(result, list) and len(result) > 0 and isinstance(result[0], dict):
            ...
            if result:
                ...
                # Add rows
                for item in result:
                    table.add_row(*[str(item.get(key, "")) for key in result[0].keys()])
