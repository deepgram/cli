"""Usage command for viewing Deepgram usage statistics."""
...
class UsageCommand(BaseCommand):
        ...
        try:
            ...
            if last_week:
                ...
                if start_date and not validate_date_format(start_date):
                    return {"status": "error", "message": f"Invalid start date format: {start_date}"}
                if end_date and not validate_date_format(end_date):
                    return {"status": "error", "message": f"Invalid end date format: {end_date}"}
                
                console.print(f"[blue]Fetching usage from {start_date or 'beginning'} to {end_date or 'now'}...[/blue]")
            else:
    ...
    def _process_usage_result(self, result: dict, summary_only: bool, start_date: str, end_date: str) -> Dict[str, Any]:
    ...
    def _display_usage_summary(self, usage_data: Dict[str, Any], start_date: str, end_date: str) -> None:
        ...
        if total_duration:
            if isinstance(total_duration, (int, float)):
                ...
                console.print(f"  Total Duration: {hours:.2f} hours ({total_duration:,} seconds)")
            else:
