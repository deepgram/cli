...
class AuthManager:
    ...
    def login_with_api_key(self, api_key: str, project_id: str, force_write: bool = False) -> None:
        ...
        if not api_key.startswith(("sk-", "pk-")):
            console.print("[red]Warning:[/red] API key format doesn't match expected pattern")
        ...
        try:
            ...
            console.print("[green]✓[/green] Credentials stored securely in system keyring")
    ...
    def login_with_device_flow(self) -> None:
        ...
        try:
            ...
            console.print(f"[dim]Verification URL:[/dim] {device_response.verification_uri}")
            console.print(f"[dim]Expires in:[/dim] {device_response.expires_in} seconds")
            ...
            try:
                ...
                console.print("Please manually navigate to the verification URL")
            ...
            console.print("[green]✓[/green] Successfully logged in via device flow")
    ...
    def _request_device_code(self) -> DeviceCodeResponse:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                raise AuthenticationError(f"Device code request failed: {response.status_code}")
    ...
    def _store_token(self, token_response: TokenResponse) -> None:
        ...
        try:
            keyring.set_password("deepgram", "access_token", token_response.access_token)
            keyring.set_password("deepgram", "api_key", api_key)
            console.print("[green]✓[/green] Token stored securely in system keyring")
        except Exception as e:
            console.print(f"[yellow]Warning:[/yellow] Could not store in keyring: {e}")
    ...
    def list_profiles(self) -> Dict[str, Dict[str, Any]]:
        ...
        for profile_name in self.config.list_profiles():
            ...
            profiles[profile_name] = {
                "api_key": "****" + profile.api_key[-4:] if profile.api_key else None,
