...
class PluginManager:
    ...
    def _load_builtin_commands(self, cli_group: click.Group) -> None:
        ...
        for command_file in command_files:
            try:
                # Import the module
                module_name = f"deepgram_cli.commands.{command_file.stem}"
                ...
                for name, obj in inspect.getmembers(module):
                    if (inspect.isclass(obj) and 
                        ...
                        console.print(f"[dim]Loaded command:[/dim] {command_instance.name}")
                        
            except Exception as e:
                console.print(f"[red]Error loading command from {command_file}:[/red] {e}")
    
    def _load_external_plugins(self, cli_group: click.Group) -> None:
        """Load external plugins from entry points."""
        try:
            # Load plugins from entry points
            for entry_point in pkg_resources.iter_entry_points("deepgram_cli.commands"):
                try:
                    ...
                    console.print(f"[dim]Loaded external plugin:[/dim] {entry_point.name}")
                    
                except Exception as e:
                    console.print(f"[red]Error loading plugin {entry_point.name}:[/red] {e}")
